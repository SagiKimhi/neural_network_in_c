...
/* Adds all required implementations */
#ifndef STANDALONE_NN_IMPLEMENTATION
...
#endif /* STANDALONE_NN_IMPLEMENTATION */
...
/* Allocation Methods */
extern nn_t
nn_alloc(size_t layers_architecture[], size_t architecture_arr_size);
extern void
nn_free(nn_t nn);

/* Value Setters: */
extern void
nn_rand(nn_t nn, float low, float high);
extern void
nn_memset(nn_t nn, float val);
...
nn_mini_batch_process(
    ...
    float rate);
extern void
nn_learn(nn_t nn, nn_t grad, float rate);
extern void
nn_back_propagation(nn_t nn, nn_t grad, nn_matrix_t ts_in, nn_matrix_t ts_out);
extern void
nn_finite_difference(
    nn_t nn, nn_t grad, float eps, nn_matrix_t ts_in, nn_matrix_t ts_out);

/* Cost and forwarding methods */
extern float
nn_cost(nn_t nn, nn_matrix_t ts_in, nn_matrix_t ts_out);
extern void
nn_forward(nn_t nn);

/* Util Methods */
extern void
nn_print(nn_t nn, const char *name);
extern void
nn_fprint(FILE *stream, nn_t nn, const char *name);
extern void
nn_save_model(FILE *fp, nn_t nn);
extern nn_t
nn_load_model(FILE *fp);
...
nn_alloc(size_t layers_architecture[], size_t architecture_arr_size)
{
    ...
    for (size_t i = 1; i < architecture_arr_size; i++) {
        ...
        nn.weights[i - 1] = nn_matrix_alloc(
            nn.activations[i - 1].cols, layers_architecture[i]);
    }
