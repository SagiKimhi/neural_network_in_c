/* Training Configuration - must be defined before including header */
#define NN_ACT_FUNC ACT_SIGMOID
#define NN_RELU_PARAM 0.01f
/*#define NN_BACKPROP_TRADITIONAL*/

/* NNC is a header library, define NN_RENDER_IMPLEMENTATION to include the
 * implementation source code in the including source file */
...
void
render_results(Rectangle frame, nn_t nn);
float
render_scrollbar(Rectangle frame);
void
...
{
    size_t arch[] = {input_size, 13, 16, 21, 16, 13, output_size};
        ...
        for (size_t y = 0; y < img.height; y++) {
            for (size_t x = 0; x < img.width; x++) {
                ...
                NN_MATRIX_AT(*ts_inout, (row_offset + pixel_idx), input_size)
                    = (pixel / 255.f);
                ImageDrawPixel(
                    &input_imgs[i], x, y, (Color){pixel, pixel, pixel, 255});
            }
        }
...
render_results(Rectangle frame, nn_t nn)
{
    ...
    for (size_t y = 0; y < input_imgs[0].height; y++) {
        for (size_t x = 0; x < input_imgs[0].width; x++) {
            ...
            NN_MATRIX_AT(NN_INPUT(nn), 0, 0)
                = (float)x / (input_imgs[0].width - 1);
            NN_MATRIX_AT(NN_INPUT(nn), 0, 1)
                = (float)y / (input_imgs[0].height - 1);
    ...
    if (!textures_were_loaded) {
        NN_ASSERT(
            (input_textures = NN_MALLOC(sizeof(Texture2D) * nof_input_imgs)));
    ...
    for (size_t i = 0; i < nof_input_imgs; i++) {
        ...
        DrawTextureEx(
            input_textures[i], input_texture_pos, 0, input_texture_scale, WHITE);
    }
    ...
    DrawTextureEx(
        actual_texture, actual_texture_pos, 0, actual_texture_scale, WHITE);
...
render_scrollbar(Rectangle frame)
{
    ...
    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)
        && CheckCollisionPointRec(mouse_pos, hitbox)) {
    ...
    return (
        (nof_input_imgs - 1)
        * (1
           - (img_selector_pos.x - bar_right_edge) / (bar.x - bar_right_edge)));
}
...
print_results(nn_t nn, nn_matrix_t ts_in, nn_matrix_t ts_out)
{
    ...
    memset(filename, 0, NN_SIZEOF_ARR(filename));
    ...
    fprintf(stdout, "Please provide a path to save the image to: ");

    if (nn_io_get_line(filename, FILENAME_MAX, stdin) <= 0) {
        return;
    }
        ...
        for (size_t i = 0; i <= 200; i++) {
            ...
            for (size_t y = 0; y < out_img_height; ++y) {
                for (size_t x = 0; x < out_img_width; ++x) {
                    NN_MATRIX_AT(NN_INPUT(nn), 0, 0)
                        = (float)x / (out_img_width - 1);
                    NN_MATRIX_AT(NN_INPUT(nn), 0, 1)
                        = (float)y / (out_img_height - 1);
