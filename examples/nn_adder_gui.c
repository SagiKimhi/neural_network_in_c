...
void
init_ts_values(nn_matrix_t *ts_inout);
void
print_model_results(nn_t nn, nn_matrix_t ts_in, nn_matrix_t ts_out);
...
main(int argc, char **argv)
{
    ...
    size_t arch[]
        = {2 * BITS, 3 * BITS, 3 * BITS, 3 * BITS, 3 * BITS, BITS + 1};
...
init_ts_values(nn_matrix_t *ts_inout)
{
    ...
    for (size_t n = 0; n < n_samples; n++) {
        ...
        for (size_t col = 0; col < BITS; col++) {
            ...
            NN_MATRIX_AT(*ts_inout, n, BITS + col)
                = ((y >> (BITS - 1 - col)) & 1);
...
print_model_results(nn_t nn, nn_matrix_t ts_in, nn_matrix_t ts_out)
{
    ...
    for (size_t n = 0; n < n_samples; n++) {
        ...
        for (size_t col = 0; col < BITS; col++) {
            NN_MATRIX_AT(NN_INPUT(nn), 0, col) = ((x >> (BITS - 1 - col)) & 1);
        ...
        if (actual_undetermined) {
            printf(
                "<Expected: %zu | Actual: X (Undetermined)>\n",
                expected_result);
            err_cnt++;
            continue;
        }
    ...
    printf(
        "Total number of model errors caught was: %zu/%zu\n",
        err_cnt,
        n_samples);
    printf("\n\n-------------------------------------------------------\n\n");
}
